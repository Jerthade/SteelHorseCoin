...
const char * const BITCOIN_CONF_FILENAME = "steelhorsecoin.conf";
const char * const BITCOIN_PID_FILENAME = "steelhorsecoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "steelhorsecoin" is a composite category enabling all SteelHorseCoin-related debug output
                if(ptrCategory->count(std::string("steelhorsecoin"))) {
...
#else
    const char* pszModule = "steelhorsecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SteelHorseCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SteelHorseCoin
    // Mac: ~/Library/Application Support/SteelHorseCoin
    // Unix: ~/.steelhorsecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SteelHorseCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SteelHorseCoin";
#else
    // Unix
    return pathRet / ".steelhorsecoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty steelhorsecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override steelhorsecoin.conf
